...
class Utils
{
    ...
    public static function throwIfInvalidStoryMentions(
        array $storyMentions)
    {
        $requiredKeys = ['user_id', 'is_sticker'];
    ...
    public static function throwIfInvalidStoryHashtags(
         ...
         array $hashtags)
    {
        $requiredKeys = ['tag_name', 'is_sticker'];
        ...
        foreach ($hashtags as $hashtag) {
            $missingKeys = array_keys(array_diff_key(['tag_name' => 1, 'is_sticker' => 1], $hashtag));
            if (count($missingKeys)) {
    ...
    protected static function _throwIfInvalidStoryStickerPlacement(
        ...
        $type)
    {
        $requiredKeys = ['x', 'y', 'z', 'width', 'height', 'rotation', 'type'];

        // Ensure that all required hashtag array keys exist.
        $missingKeys = array_keys(array_diff_key(['x' => 1, 'y' => 1, 'z' => 1, 'width' => 1, 'height' => 1, 'rotation' => 1, 'type' => 1], $storySticker));
        if (count($missingKeys)) {

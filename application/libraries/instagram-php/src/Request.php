...
class Request
{
    ...
    /**
     * Whether this API call needs to silent exceptions.
     *
     * Off by default.
     *
     * @var bool
     */
    protected $_silentFail;
    ...
    /**
     * Version when a fuctions becomes deprecated.
     *
     * @var string
     */
    protected $_deprecatedVersion;
        ...
        $url)
    {
        ...
        $this->_parent->_needsAuth = true;
        $this->_signedPost = true;
        $this->_signedGet = false;
        $this->_silentFail = false;
    ...
    protected function _addDefaultHeaders()
    {
        if ($this->_defaultHeaders) {
            ...
            $this->_headers['X-IG-App-Startup-Country'] = Constants::APP_STARTUP_COUNTRY;
            $this->_headers['X-IG-VP9-Capable'] = $this->_parent->getIsVP9Compatible() ? 'true' : 'false';
            ...
            $this->_headers['X-MID'] = $this->_parent->client->getMid();
        }
    ...
    public function addCookie(
        ...
        $value)
    {
        $this->_parent->client->setCookie($name, '.instagram.com', $value);
        return $this;
    }
    ...
    public function removeCookie(
        $name)
    {
        $this->_parent->client->removeCookie($name);
        ...
        $needsAuth)
    {
        ...
        $this->_parent->_needsAuth = $needsAuth;
